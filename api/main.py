...
def stream_track(track_id: int, db=Depends(get_db), sc=Depends(get_sc)):
    """Stream a track — prefer local file URL, fall back to SoundCloud."""
    # Priority 0: Local file on disk — return a URL to the play endpoint
    row = db.conn.execute(
        "SELECT file_path FROM downloads WHERE track_id = ? AND file_path IS NOT NULL LIMIT 1",
        (track_id,)
    ).fetchone()
    if row and row["file_path"] and os.path.exists(row["file_path"]):
        return {"url": f"/api/downloads/play/{track_id}", "type": "mp3"}
